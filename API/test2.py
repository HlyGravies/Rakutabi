...
USER_LOCATION = "34.6872571,135.5258546" 
USER_RADIUS = 30000

# ⚠️ BƯỚC 1: CẬP NHẬT DANH SÁCH FIELDS
# Chúng ta phải yêu cầu các trường cha (như 'reviews', 'photos', 'opening_hours')
# để có thể lấy được các trường con (như 'text', 'photo_reference')
PLACE_DETAILS_FIELDS = [
    "place_id",           # Bắt buộc để gộp (merge)
    "opening_hours",      # Để lấy 'weekday_text'
    "photos",             # Để lấy TẤT CẢ 'photo_reference'
    "price_level",        # Bạn yêu cầu
    "rating",             # Bạn yêu cầu
    "reviews",            # Để lấy 'text' từ reviews
    "user_ratings_total"  # Bạn yêu cầu
]
...
preference_to_api_map = {
    ...
    "pref_karaoke": {"type": "night_club", "keyword": "カラオKE"},
...
def fetch_places_for_job(job, location, radius):
    ...
    # Xử lý trường hợp 'type' là một danh sách (ví dụ: ['aquarium', 'zoo'])
    types_to_search = job['type']
    if not isinstance(types_to_search, list):
        types_to_search = [types_to_search] # Đảm bảo nó luôn là list

    for place_type in types_to_search:
        params = {
            'location': location,
            'radius': radius,
            'type': place_type, # Tìm kiếm cho từng type
        ...
        # print(f"[NearbyWorker] Bắt đầu job: type={place_type}, keyword={job['keyword']}...")
...
# 🏁 BƯỚC CUỐI: Chạy thử, LỌC VÀ LƯU FILE
if __name__ == "__main__":
    
    # Giả lập User chọn 3 sở thích
    ...
    # --- BƯỚC 2 (MỚI): LỌC KẾT QUẢ CUỐI CÙNG ---
    ...
    if full_data_places:
        for place in full_data_places:
            ...
            # 8. photo_references (Từ Phase 2)
            # Tạo một danh sách chỉ chứa các photo_reference
            ...
            # 9. review_texts (Từ Phase 2)
            # Tạo một danh sách chỉ chứa nội dung 'text'
    ...
    if minimal_results_list:
        ...
        FILENAME = f"MinimalSearch_{safe_prefs}_{timestamp}.json" # Đổi tên file
        ...
        try:
            ...
            # Lưu danh sách đã lọc (minimal_results_list)
